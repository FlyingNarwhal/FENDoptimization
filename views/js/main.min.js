...
changeSliderLabel(size);function changePizzaSizes(size){var randPizza=document.getElementsByClassName('randomPizzaContainer');switch(size){case"1":newwidth='25';break;case"2":newwidth='33.3';break;case"3":newwidth='50';break;default:console.log("bug in sizeSwitcher");}
for(var i=0;i<randPizza.length;i++){randPizza[i].style.width=newwidth+'%';}}
...
var items=document.getElementsByClassName('mover');function updatePositions(){frame++;window.performance.mark("mark_start_frame");var scroll=document.body.scrollTop;for(var i=0;i<items.length;i++){var phase=Math.sin((scroll/1250)+(i%5));items[i].style.left=items[i].basicLeft+100*phase+'px';}
...
window.addEventListener('scroll',updatePositions);document.addEventListener('DOMContentLoaded',function(){var cols=8;var s=256;var height=window.screen.height;var row=height/s;var pizzaQuantity=row*cols;for(var i=0;i<pizzaQuantity;i++){var elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height="100px";elem.style.width="73.333px";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i/cols)*s)+'px';document.querySelector("#movingPizzas1").appendChild(elem);}
updatePositions();});